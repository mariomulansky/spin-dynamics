...
public:
    ...
    spin_stepper( int N , 
                  ...
                  vector_type &h_y , 
                  vector_type &h_z  )
        : m_N( N ) , m_J( J ) , m_h_x( h_x ) , m_h_y( h_y ) , m_h_z( h_z ) ,
          m_b_x( N/2 ) , m_b_y( N/2 ) , m_b_z( N/2 ) , m_b_norm( N/2 )
    { }
...
/* compute /vec b and the norm B */
        thrust::for_each( 
            ...
            thrust::make_zip_iterator( thrust::make_tuple( 
                 ...
                 m_b_z.end() ,
                 m_b_norm.end() ) ) ,            
            b_functor< value_type >( m_J ) );        
    }

    void evolve_s( vector_type &s_x , vector_type &s_y , vector_type &s_z , const double dt ,
                   const bool even )
    {
        int m;
        if( even ) m = 0;
