...
public:
    ...
    template< class VectorType >
    void init_gaussian_random( const ValueType sigma , VectorType &x , VectorType &y , VectorType &z )
    {
        for( int n=0 ; n<x.size() ; ++n )
        {
            x[n] = static_cast< value_type >( rand() ) / RAND_MAX - 0.5; 
            ...
            z[n] = static_cast< value_type >( rand() ) / RAND_MAX - 0.5;
            value_type nrm = sqrt( x[n]*x[n] + y[n]*y[n] + z[n]*z[n] );
            value_type r = sqrt( std::abs(sigma*gen_normal()) );
            x[n] *= r/nrm;
            y[n] *= r/nrm;
            z[n] *= r/nrm;            
        }
